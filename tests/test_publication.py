...
def test_publication_audit_rejects_positioning_language_in_public_docs(tmp_path):
    from scripts.check_publication import audit_tree

    (tmp_path / "README.md").write_text("# Product\nBuilt as an AI-assisted interview showcase.\n")
    violations = audit_tree(tmp_path, [Path("README.md")])
    assert violations == ["positioning language in README.md: AI-assisted, interview, showcase"]
